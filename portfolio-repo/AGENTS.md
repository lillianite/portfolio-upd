...
A 2D free-roaming infinite-canvas portfolio for designer Lillian Navales Rivera. The entire UI lives at `/` — a full-screen draggable/zoomable canvas with spatially arranged content nodes (biography, project case study cards, PDF viewer). Built with TanStack Start and deployed on Netlify.

## Infinite Canvas Architecture

`src/components/InfiniteCanvas.tsx` is the root component. It renders a 5000×5000 px content layer inside `react-zoom-pan-pinch`'s `TransformWrapper`. Nodes are placed absolutely, offset from the canvas center (2500, 2500) via the `CanvasNode` helper: `left: 2500 + x, top: 2500 + y, transform: translate(-50%,-50%)`. Node positions live in `NODE_POSITIONS` in that file.

Canvas sub-components live in `src/components/canvas/`:
- `BiographyNode.tsx` — static bio/contact card
- `ProjectNode.tsx` — clickable project card (fires `ProjectModal`)
- `ProjectModal.tsx` — full-screen overlay for project detail; closes on Escape or backdrop click
- `PdfNode.tsx` — collapsible inline PDF iframe; expands in-place to avoid z-index issues
- `CanvasControls.tsx` — fixed-position zoom +/− and reset-to-center buttons
- `CanvasDotGrid.tsx` — SVG dot-pattern background

`overflow: hidden` on `<body>` in `__root.tsx` prevents scroll bars.

## Adding or Moving Nodes

Edit `NODE_POSITIONS` in `InfiniteCanvas.tsx`. To add a new project, drop a `.md` file in `content/projects/` — it is picked up automatically by content-collections and rendered as a `ProjectNode`.

## Content Schema (projects)

Fields: `title`, `description`, `tags[]`, `year?`, `role?`, `image?`, `github?`, `liveUrl?`, `content`
